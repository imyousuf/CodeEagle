...
func TestABurstOfChangesCostsOneSync(t *testing.T) {
	...
	for i := range 10 {
		tr.Touch("a", t0.Add(time.Duration(i)*time.Second))
	}
...
func TestAChangeDuringASyncSurvivesIt(t *testing.T) {
	...
	tr.Touch("a", mid)
	tr.Finished("a", t0.Add(60*time.Second), nil, time.Minute)
...
func TestSuccessClearsTheProject(t *testing.T) {
	...
	tr.Touch("a", t0)
	tr.Started("a")
	tr.Finished("a", t0.Add(time.Minute), nil, time.Minute)
...
func TestFailureBacksOffAndRetries(t *testing.T) {
	...
	tr.Finished("a", done, boom, time.Minute)
...
func TestBackoffGrowsWithConsecutiveFailures(t *testing.T) {
	...
	for range 3 {
		...
		tr.Finished("a", now, boom, time.Minute)
	}
...
func TestSuccessResetsTheBackoff(t *testing.T) {
	...
	tr.Started("a")
	now = now.Add(time.Second)
	tr.Finished("a", now, errors.New("x"), time.Minute)

	tr.Touch("a", now.Add(2*time.Minute))
	tr.Started("a")
	now = now.Add(3 * time.Minute)
	tr.Finished("a", now, nil, time.Minute)
...
func TestOldestChangeIsOfferedFirst(t *testing.T) {
	tr := newTracker(time.Second)
	tr.Touch("late", t0.Add(10*time.Second))
	tr.Touch("early", t0)
...
func TestRunningCounts(t *testing.T) {
	...
	tr.Finished("a", t0, nil, time.Minute)
	if got := tr.Running(); got != 1 {
		t.Errorf("Running() = %d, want 1", got)
	}
}
